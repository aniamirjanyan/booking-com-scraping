...
def create_url(people, country, city, datein, dateout, offset, currency="USD"):
    ...
    for hotel in hotels:
        ...
        if hotel_info['link'] is not None:
            ...
            if is_detail:
                ...
                hotel_info['important_facilities'] = core.get_important_facilites(soup_detail)
...
def retrieve_data(people, country, city, datein, dateout, outdir, is_detail, limit):
    ...
    if outdir == "":
        outdir = ("./" + country + city + "_" + datetime.datetime.now().strftime(
            "%Y-%m-%d %H:%M:%S") + ".json").replace(" ", "_").replace(":", "_")
