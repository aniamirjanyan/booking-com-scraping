...
def get_important_facilites(soup_detail):
    if soup_detail.select_one("div.hp_desc_important_facilities.clearfix.hp_desc_important_facilities--bui") is None:
        ...
        return list(dict.fromkeys(
            [service.text.strip() for service in soup_detail.findAll("div", {"class": "important_facility"})]))

# ===============================
# def get_square_meter(soup_detail):
#     # if soup_detail.find_all("div.hprt-facilities-facility") is None:
#     #     return "not cool"
#     try:
#         # res = soup_detail.find_all(name='div', class_='hprt-facilities-facility')[0].find("span").text
#         return soup_detail.find("span", {"class": "hprt-facilities-facility", "data-value": True})['data-value']
#         # return str(res)
#     except:
#         return " "
# find_all(name='div', class_='indent--FyTYr')
# ===============================
...
def get_neighborhood_structures(soup_detail):
    ...
    if soup_detail.select_one('div.hp-poi-content-container.hp-poi-content-container--column.clearfix') is None:
        ...
        for neighborhood in soup_detail.select_one(
...
def get_services_offered(soup_detail):
    ...
    if soup_detail.select_one('div.facilitiesChecklist') is None:
        ...
        for services in soup_detail.findAll("div", class_="facilitiesChecklistSection"):
            ...
            services_offered = {'type': services.find("h5").text.strip(), 'value': []}

            for checks in services.findAll("li"):

                if checks.find("p") is not None:
                    services_offered['value'].append(
                        checks.findNext("p").text.strip().replace("\n", " ").replace("\r", " ").replace("  ", " "))
